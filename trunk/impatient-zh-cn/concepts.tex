...
\bookmark{2}{从A到B}
\conceptindex{活动字符 (active characters)}
\concept {活动字符 (active character)}

一个\defterm{活动字符}是一个有定义的\refterm{字符}.
比如一个活动字符可能表示着一个宏定义.
...
一个活动字符的\refterm{类码}为 $13$ (^|\active|的值).
当你定义一个活动字符时,
...
它可以在所连接的两个单词当中产生一个空格, 并且不让 \TeX\ 把这个空格转为 \refterm{断行}. \refterm{\PlainTeX:\plainTeX} 通过以下方式来定义 `|~|':
...
\bix^^{tables}
一个排列是一个把素材有序排印的结构, 比如一张有行有列的表格.
当你需要构成一个排列时, 你要 (a)~描述行和列的布局, 并且 (b)~告诉 \TeX\ 插在行或者列中的内容.
表排列和水平排列是由一系列的行组成; 垂直排列则由一系列的列组成.
我们首先讲述表排列和水平排列, 然后简要地介绍一下垂直排列.

表排列是在 \plainTeX\ 中定义的. 它很简单, 因此不如水平排列那样灵活.
表排列和水平排列的主要区别在于你如何定义它们的布局.
