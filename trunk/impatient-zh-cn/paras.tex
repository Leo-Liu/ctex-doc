...
一般情况下，这些字体风格命令和相应的文本被一起包含在一个编组里。如果放在编组之外，效果将会作用于文档剩余部分（除非用另外一个这样的命令覆盖它）。
\example
The Dormouse was {\it not} amused.
|
\produces
...
%\enddesc
改变|\spacefactor|的值，就可以方便地调整单词间距。
默认的单词间距通过|\xspaceskip|或者|\spaceskip|来改变：
\ulist
\li |\xspaceskip|设定$f\ge2000$时的粘连；如果|\xspaceskip|是零, 就取默认值。
\li |\spaceskip|设定$f<2000$或者
\hbox{|\xspaceskip|}为零时的粘连；如果|\spaceskip|是零，取默认值。
|\spaceskip| 粘连的拉伸或压缩依$f$的值而变，就像普通的单词间距一样。
\endulist

\knuth{page~76} 对\TeX\ 计算单词间\minref{粘连}的规则有一个具体的解释。
\knuth{pages~285--287}详细描述了水平列表中不同项目之后|\spacefactor|的计算、修正。
\eix^^{spaces//interword}
\enddesc
...
^^{indentation} 你可以通过设置|\leftskip|为一个固定的非零\minref{尺寸}来增加左边距。
...
因为 \TeX\ 通常不愿意在其中断行。^^{line breaks//in math formulas}
此命令也能用于竖直模式。
\example
