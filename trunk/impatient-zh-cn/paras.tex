...
如果两者不同，\TeX\ 添加一个 ^{whatsit}，表明当前语言的变化。
这个 whatsit 提示后续处理时所用的语言规则应该改变。
\enddesc
...
此命令通过添加 whatsit 设定当前语言值为 \<number>。
该 whatsit 和修改 ^|\language| 时的一样。
...
而不是字体文件名 \<font\-name>。
谨记：在群组结束时所分配的 |\hyphenchar| \emph{不会}撤销。
